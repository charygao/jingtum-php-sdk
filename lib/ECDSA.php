...
class ECDSA
{
    ...
    {
        if (! empty($secret) && ! $this->validateWifKey($secret)) {
     ...
     * @return string (hexa)
     */
    public function halfhash512($data)
    {
        return substr(hash('sha512', $data), 0, 64);
    }
     ...
     * @throws \Exception
     */
    public function generateRandom256BitsHexaString($seed, $extra = 'FkejkzqesrfeifH3ioio9hb55sdssdsdfOO:ss')
    {
        $seq = 0;
        ...
        do {
          if ($seed == '') {
            ...
            $bytes = openssl_random_pseudo_bytes(256, $cStrong);
                ...
                $seed = substr(bin2hex($this->hash256($random)), 0, 32);
            }
            
            $this->seed = $seed;
            
            $data = hex2bin($seed) . hex2bin(sprintf("%08x", $seq));
            
            $res = $this->halfhash512($data);
    ...
    public function base58_decode($encodedData, $littleEndian = true)
    {
        ...
        for ($i = $length - 1; $i >= 0; $i --) {
            $res = gmp_add(gmp_mul($res, gmp_init(58, 10)), $this->base58_permutation(substr($encodedData, $i, 1), true));
        }
        
        $res = gmp_strval($res, 16);
        $i = $length - 1;
        while (substr($encodedData, $i, 1) === 'j') {
            $res = '00' . $res;
            $i --;
        }
...
//        $secretKey = '21' . $k;
    ...
    /**
     * *
     * Computes the result of a point addition and returns the 
     * resulting point as an Array.
     *
     * @param Array $pt            
     * @return Array Point
     * @throws \Exception
                ...
                if ($derEvenOrOddCode === '03') // odd
{
     ...
     * @throws \Exception
     */
    public function getPubKeyPoints($secret, $base)
    {
        $G = $this->G;
...
//          printf("key value is set[%s]\n",$this->k);
    ...
    {
        $this->private_gen = $this->generateRandom256BitsHexaString('', $extra);
    }
    ...
    public function getSignatureHashPoints($hash, $nonce = null)
    {
        ...
        if ($nonce === null) {
            $nonce = gmp_strval(gmp_init($this->generateRandom256BitsHexaString(''), 16), 16);
        }
        ...
        $R = gmp_strval(gmp_mod($rPt['x'], $n), 16);
        
        // second part of the signature (S).
        // S = nonce^-1 (hash + privKey * R) mod p
        
        $S = gmp_strval(gmp_mod(gmp_mul(gmp_invert(gmp_init($nonce, 16), $n), gmp_add(gmp_init($hash, 16), gmp_mul($k, gmp_init($R, 16)))), $n), 16);
